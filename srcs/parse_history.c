...
/*   Updated: 2016/09/26 18:29:55 by jguthert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		transform_str(t_line *l, char *new_entry, int i)
{
	...
	while (l->str[i] != '\0' && ft_isblank(l->str[i]) == 0)
		i++;
	temp = ft_strjoin(new_entry, l->str + i);
	if (temp == NULL)
		return (1);
	ft_strdel(&(l->str));
	l->str = temp;
...
void			parse_history(t_line *l, t_ftl_root *hist)
{
	...
	{
		if (l->str[i] == '!' &&
			(i == 0 || (l->str[i - 1] != '\\' && l->str[i - 1 != '!'])))
		{
			new_entry = get_event(l->str + i, hist);
			if (new_entry != NULL)
			{
