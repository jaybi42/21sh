...
/*   Updated: 2016/12/06 18:50:33 by agadhgad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ret_o_b_place(t_arr *r, char **a_t, int i_t)
{
	...
	if (!(old_tmp = x_strdup2((r->t_s)[(r->t_i) - 1],
					(r->t_a)[(r->t_i) - 1])))
		return (FALSE);
	i_t = 0;
	while (a_t[i_t] != NULL)
	{
		...
		{
			(r->t_s)[(r->t_i) - 1 + i_t] = x_strjoin2((r->t_s)[(r->t_i) -
					1 + i_t],
		...
		{
			(r->t_s)[(r->t_i) - 1 + i_t] = x_strjoin2(old_tmp,
					ft_strlen(old_tmp), a_t[i_t], ft_strlen(a_t[i_t]));
...
int		ret_other(t_arr *r, char *s)
{
	...
	while (i_tmp < (r->tog))
	{
		(r->t_s)[(r->t_i) - 1 - i_tmp][(r->t_a)[(r->t_i) - 1 - i_tmp]++] =
			s[(r->i)];
