...
int 	parse_glob_rect(t_norm_glob *g)
{
	int tmp_len;
	tmp_len = 0;
	if (((g->g_tmp) = quarter((g->exprs)[(g->ta)] + (g->gti)[(g->ta)] + 1, &tmp_len)) != NULL)
	{
		(g->tgs)[(g->ta)][(g->gta)[(g->ta)]++] = (g->g_tmp);
		(g->gti)[(g->ta)] += tmp_len + 1;
	}
	else if (((g->g_tmp) = rec_g((g->exprs)[(g->ta)], &(g->gti)[(g->ta)], 0)) != NULL)
		(g->tgs)[(g->ta)][(g->gta)[(g->ta)]++] = (g->g_tmp);
	else
		return (FALSE);
	return (TRUE);
...
int		parse_glob_globbing(t_norm_glob *g)
{
	while ((g->exprs)[(g->ta)][(g->gti)[(g->ta)]])
	{
		...
		else if (((g->g_tmp) = rec_g((g->exprs)[(g->ta)], (g->gti) + (g->ta), 1)) != NULL)
			(g->tgs)[(g->ta)][(g->gta)[(g->ta)]++] = (g->g_tmp);
		else
			return (FALSE);
		(g->gti)[(g->ta)]++;
